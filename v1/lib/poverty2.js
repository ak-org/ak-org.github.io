...
function makeBar(svgId, stateName) {
  ...
  var margin = {top: 20, right: 20, bottom: 120, left: 40},
      ...
      console.log("Bar Chart for state", stateName);
  ...
  var bartip = d3.tip()
                 ...
                 .offset([-10,0])
                 .html(function(d,i) {
                    //console.log("bar tip value",d,i)
                    var pValue = d[i];
                    return "<span class='details'>" +
                            pValue + "<span class='details'>" + " %" ;
              });
   bartip.offset([-60, -10]);
  ...
  var svg = d3.select(svgId).select(".svgBar1")
   ...
   var g = d3.select(svgId).select(".svgBar1").select("g");

   console.log(data);
   svg.call(bartip);

    // Scale the range of the data in the domains
    x.domain(data.map(function(d,i) { return barChartLabels[i]; }));
    y.domain([0, 40]);
    d3.select("#barTitle1")
    ...
    g.selectAll("rect")
      ...
      .enter().append("rect")
        .attr("class", "bar")
        .attr("x", function(d,i) { return x(barChartLabels[i]); })
        ...
        .attr("y", function(d,i) {
                if (isNaN(y(data[i]))) {
                   ...
                   retHeight = height - y(data[i]);
                }
                //console.log(retHeight,d,height,y(data[i]));
                return retHeight;
            })
        .style("fill", function(d,i) { return barChartColors[i];})
        .attr("height", function(d,i) {
              retHeight = 2*height - y(data[i]);
              //console.log(retHeight);
              return
             })
        .on("mouseover", function(d,i) {
          var pValue = y(data[i]);
           console.log("bar tip(in)",pValue,d,data,i);
           bartip.show(d,i);

        })
        .on("mouseout", function(d,i) {
           //console.log("bar tip(out)",d,i);
           bartip.hide(d);
        })
      .exit()
        .remove();
...
//////////////////////// tooltip /////////////////////////
          ...
          g.selectAll("rect")
            .on("mouseover", function(d,i) {
              var pValue = y(d[i]);
              console.log("bar tip(in2)",pValue,d,data,i);
               bartip.show(data,i);
    ...
    if (axis_added == 0) {
        ...
        // don't redraw axis over and over again
        svg.append("g")
...
/////////////////// Orig function /////////////////////////
function makeBarOrig(svgId, stateName) {
...
function makeBar2(svgId, stateName,incomingdata) {
   ...
   var ordinal = d3.scaleOrdinal()
    ...
    console.log("Create HD Bar Chart for ", stateName, "with values ", data);
    var svg = d3.select(svgId).select(".svgBar2")
        .attr("width", width + margin.left + margin.right)
        .attr("height", height + margin.top + margin.bottom)
        .append("g")
        .attr("transform",
     ...
     var g = d3.select(svgId).select(".svgBar2").select("g");

     console.log(data);

     // Scale the range of the data in the domains
     x.domain(data.map(function(d,i) { return hgLegends[i]; }));
     y.domain([50, 600]);
     ...
     d3.select("#barTitle2")
      ...
      var bar2tooltip = d3.select("#barTitle2").append("div")
          .attr("class", "bar2tooltip")
          .style("opacity", 0);
      ...
      g.selectAll("rect")
          ...
          .append("rect")
          .attr("class", "bar")
          .attr("x", function(d,i) { return x(hgLegends[i]); })
          .attr("width", x.bandwidth())
          .attr("y", function(d,i) {
                        if (!isNaN(y(data[i]))) {
                          return y(data[i]);
          ...
          .on("mouseover", function(d,i) {
              bar2tooltip.transition()
                         .duration(200)
                         .style("opacity", .9);

              console.log("Value is ", d,i,d3.event.pageX,d3.event.pageY);
              bar2tooltip.html("<br/>" + data[i] + " per 100,000 Adults")
                          .style("top", d3.event.pageY - 750 + "px")
...
/////////////// tooltip function /////////////////////////
          ...
          g.selectAll("rect")
            .on("mouseover", function(d,i) {
              ...
              console.log("Value 2 is ", d,i,d3.event.pageX,d3.event.pageY);
              bar2tooltip.html("<br/>" + data[i] + " per 100,000 Adults")
                         .style("top", (y(data[i])+20) + "px")
                         .style("left", (x(hgLegends[i]) + 20) + "px");
                         //.style("top", d3.event.pageY - 750 + "px")
                         //.style("left", d3.event.pageX - 700  + "px");
      ...
      if (axis_added_2 == 0) {
          console.log("adding transform again")
          axis_added_2 = 1;
...
function makeBar2Orig(svgId, stateName,incomingdata) {
